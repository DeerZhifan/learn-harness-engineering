...
interface DocumentStatusUpdater {
  updateDocument: (id: string, updates: Partial<Document>) => Document | null;
}

export class IndexingService {
  private persistence: PersistenceService;
  private documentService: DocumentStatusUpdater | null;

  constructor(persistence: PersistenceService, documentService?: DocumentStatusUpdater) {
  ...
  /** Get current indexing status with chunk counts. */
  getStatus(): IndexStatus {
    ...
    for (const docId of Object.keys(chunksMeta)) {
      totalChunks += this.getChunksForDocument(docId).length;
    }
  ...
  /** Renderer-facing app status. */
  getAppStatus(): AppStatus {
    const s = this.getStatus();
    return {
      documentsLoaded: s.totalDocuments,
      indexStatus: s.status,
      lastActivity: s.lastIndexed ?? '',
      indexedCount: s.currentIndexed,
